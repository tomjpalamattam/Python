...
def Country_Association_all(df_1):
...
def distribution_CA(list1,df_2):    
    appended_data=[]
    for item1 in list1:
        print('\n checking for Country:',item1)
        temp_df= df_2.loc[df_2['Country'].str.contains(item1, case=False, na=False)] #creates a temporary dataframe of the same 'Country'
        tt_df=temp_df['Country Association'].value_counts().to_frame() # prints column 'Country Association' variation for the respective 'Country' in dataframe 'temp_df' 
        print(tt_df)   
...
#list2=Country_Association_all(df)
list2=['India', 'Belgium (BE)', 'Belarus','Czechia','Zimbabwe']
distribution_CA(list2,df)
