...
def process_with_openrouter(job_description, project_data):
    prompt = f"""
    ROLE: You are a strict resume editor optimizing for ATS compliance.
    TASKS:
    1. Analyze this job description's key requirements
    2. Rewrite ONLY ACTUAL PROJECTS from provided data using business language
...
def process_with_openrouter_delete_sessions(job_description, project_data):
    ...
    Choose whether a project is relevant or not. You should still keep projects if its related to the job domain. You are only supposed to give a boolean answer (0 or 1).
...
def process_with_deepseek_delete_sessions(job_description, project_data):
    ...
    Choose whether a project is relevant or not. If the project has anything relevant to the job description, you should keep the project.
    For example, if a project is from AI domain and so is the job you should keep it.
    You are only supposed to give a boolean answer (0 or 1).
...
def update_skills_section_old(doc, skills):
    ...
    skills_heading = next((p for p in cell.paragraphs if "Skills" in p.text), None)

    if not skills_heading:
        skills_heading = cell.add_paragraph("Skills")
...
"""
# Load the document
doc = Document("templates/CV-template.docx")
...
cell_0_1_data = [item['text'] for item in cell_0_1_content
          if (item['text'] == "Skills" and (start := True)) or
             (start and item['text'] != "Languages" and not (item['text'] == "Languages" and False))]
