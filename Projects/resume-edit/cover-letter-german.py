...
def process_with_deepseek(job_description, matter):
    ...
    - Analyze this job description's key requirements in German to ensure ATS passing.
    ...
    - The whole content should be written in german
    -  Return only the body content without any opening/closing salutations.
...
def process_with_openrouter(job_description, matter):
    ...
    - Flow naturally from the opening and conclude approppriately before the closing.
    - The whole content should be written in german
    - Return only the body content in paragraphs without any opening/closing salutations.
...
def save_cover_letter(content, output_path="generated_cover_letter.docx"):
    ...
    # Add closing
    closing_para = doc.add_paragraph("Mit freundlichen Grüßen,")
